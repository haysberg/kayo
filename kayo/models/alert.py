...
class Alert(BaseModel):
    ...
    async def send_alert(self, match : Match):
        ...
        if match.team1 in instance.referential["teams"] and instance.referential["teams"][match.team1] != "":
            embed.add_field(name=f'{match.team1}\'s stream', value=f'[Link]({instance.referential["teams"][match.team1]})', inline=True)

        embed.add_field(name="Check on vlr.gg", value=f'[Click here](https://vlr.gg{match.match_page})', inline=True)

        if match.team2 in instance.referential["teams"] and instance.referential["teams"][match.team2] != "":
            embed.add_field(name=f'{match.team2}\'s stream', value=f'[Link]({instance.referential["teams"][match.team2]})', inline=True)
